      ...
      .then((data) => {
        ...
        if (data.length > 0) {
          data.forEach((product) => {
            ...
            row.innerHTML = `
                        <td class="p-3 border-b" data-label="Image">
                            <img src="image.php?id=${
                              product.id
                            }" alt="Product Image" width="50" height="50" class="rounded-full">
                        </td>
                        <td class="p-3 border-b" data-label="Product ID">${highlight(
                          product.ProductId,
                          query
                        )}</td>
                        <td class="p-3 border-b" data-label="Name">${highlight(
                          product.ProductName,
                          query
                        )}</td>
                        <td class="p-3 border-b" data-label="Units">${highlight(
                          product.ProductUnits,
                          query
                        )}</td>
                        <td class="p-3 border-b" data-label="Price">${
                          product.Price
                        }</td>
                        <td class="p-3 border-b" data-label="Stocks">
                            <span style="${stockStyle}">${stockText}</span>
                        </td>
                        <td class="p-3 border-b" data-label="Type">${
                          product.ProductType
                        }</td>
                        <td class="p-3 border-b flex gap-2 relative" data-label="Actions">
  ...
  function handleDeleteClick(event) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`../p/delete_product.php?id=${productId}`, { method: "DELETE" })
          .then((response) => response.json())
          .then(() => {
            Swal.fire({
              title: "Deleted!",
